...
{
    // Uzupelnij cialo funkcji set_handler zgodnie z komentarzem powyzej.
    struct sigaction action;

    action.sa_sigaction = &handler;  // Funckaj handler obsluguje sygnal.
    action.sa_flags = SA_SIGINFO;  // Funkcja handler odczytuje pewna dodatkowa informacje wyslana z sygnalem.

    sigemptyset(&action.sa_mask);
    sigaddset(&action.sa_mask, SIGPIPE); // W trakcie obslugi sygnalu SIGUSR1 bedzie blokowany SIGPIPE.

    sigaction(SIGUSR1, &action, NULL);  // Funkja obsluguje sygnal SIGUSR1.
}
