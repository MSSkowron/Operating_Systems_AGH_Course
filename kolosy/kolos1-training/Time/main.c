...
/* wykonaj program, policz jego czasy, sprawdz,czy to nie jest ostatni program */
{
    ...
    if ((start = (times(&tms_start))) ==-1) /* zapamietaj wartosci poczatkowe pomiaru czasu*/
    {
        printf("times error");
        exit(-1);
    }
    ...
    if ((end = (times(&tms_end))) == -1) /* zapamietaj wartosci koncowe */
    {
        printf("times error");
        exit(-1);
    }
    ...
    display_times(end - start, &tms_start, &tms_end);
