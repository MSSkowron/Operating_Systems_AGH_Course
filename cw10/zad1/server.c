...
int get_client_descriptor(int local_server_socket, int net_server_socket){
    struct pollfd *fds = calloc(no_connected_clients + 2, sizeof(struct pollfd));
    ...
    for(int i = 0; i < MAX_NO_CLIENTS; i++)
    {
        if(clients[i] != NULL)
        {
            // Deskryptor pliku(socket) klienta
    ...
    // sockety serwera
    fds[no_connected_clients].fd = local_server_socket;
    fds[no_connected_clients].events = POLLIN;
    fds[no_connected_clients + 1].fd = net_server_socket;
    fds[no_connected_clients + 1].events = POLLIN;
    
    // Czekamy na eventy po stronie klienta
    poll(fds, no_connected_clients + 2, -1);

    int client_id;
    for(int i = 0; i < no_connected_clients + 2; i++)
    {
        // Zaszlo zdarzenie dla i'tego klienta
    ...
    if (client_id == local_server_socket || client_id == net_server_socket)
    {
...
void remove_both_clients(int index){
    ...
    if(clients[opponent] != NULL)
    {
        ...
        send(clients[opponent]->fd, buff, MAX_MSG_LENGTH, 0);
...
void close_server(){
    for(int i = 0; i < MAX_NO_CLIENTS; i++)
    {
        if(clients[i] != NULL)
        {
            ...
            send(clients[i]->fd, buff, MAX_MSG_LENGTH, 0);
            remove_client(i);
        }
    }
    exit(0);
...
int main(int argc, char** argv)
{
    ...
    int local_server_socket = socket(AF_UNIX, SOCK_STREAM, 0);
    ...
    // Rozpoczecie akcpetowania polaczen od klientow.
    int local_listen = listen(local_server_socket, MAX_NO_CLIENTS);
    if(local_listen == -1)
    {
        perror("Error while starting to listening - local socket!\n");
        exit(0);
    }

    // Stworzenie gniazda komunikacji internetowej w oparciu o IPv4
    int net_server_socket = socket(AF_INET, SOCK_STREAM, 0);
    ...
    // Rozpoczecie akcpetowania polaczen od klientow.
    int net_listen = listen(net_server_socket, MAX_NO_CLIENTS);
    if(net_listen == -1)
    {
        perror("Error while starting to listening - net socket!\n");
        exit(0);
    }
    ...
    while(1)
    {
        ...
        recv(client_fd, buff, MAX_MSG_LENGTH, 0);
        ...
        if(strcmp(command_action, "initialization") == 0)
        {
            if(find_by_name(client_name) == -1)
            {
                ...
                if(client_index == -1)
                {
                    send(client_fd, "initialization:no_free_slot", MAX_MSG_LENGTH, 0);
                }
                else
                {
                    ...
                    if(opponent == -1)
                    {
                        send(client_fd, "initialization:no_enemy_found", MAX_MSG_LENGTH, 0);
                    }
                    ...
                    else
                    {
                        ...
                        if(number % 2 == 0)
                        {
                            send(clients[client_index]->fd, "initialization:O", MAX_MSG_LENGTH, 0);
                            send(clients[opponent]->fd, "initialization:X", MAX_MSG_LENGTH, 0);
                        }
                        else
                        {
                            send(clients[opponent]->fd, "initialization:O", MAX_MSG_LENGTH, 0);
                            send(clients[client_index]->fd, "initialization:X", MAX_MSG_LENGTH, 0);
                        }
                    }
                }
            }
            ...
            {
                send(client_fd, "initialization:username_in_use", MAX_MSG_LENGTH, 0);
            }
        }
