...
int connect_local(char *path){
    ...
    if(local_bind == -1){
        perror("Error while binding local socket\n");
        exit(0);
    }
    ...
    // Wyslij wiadomosc do serwera o podlaczeniu
    char msg[MAX_MSG_LENGTH];
    sprintf(msg, "initialization: :%s", client_name);
    sendto(server_socket, msg, MAX_MSG_LENGTH, 0, (struct sockaddr *) &local_addr, sizeof(struct sockaddr_un));
...
int connect_inet(int port){
    ...
    char msg[MAX_MSG_LENGTH];
    sprintf(msg, "initialization: :%s", client_name);
    sendto(server_socket, msg, MAX_MSG_LENGTH, 0, (struct sockaddr *) &net_addr, sizeof(struct sockaddr_in));
    return server_socket;
}
