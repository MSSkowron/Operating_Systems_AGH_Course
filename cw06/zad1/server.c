...
void save_log_to_file(msgbuf *message) {
    ...
    switch(message->mtype) {
        ...
        case TO_ALL:
            ...
            break;
        case TO_ONE:
            printf("Got TO_ONE from: %d to: %d\n", message->sender_pid, message->receiver_id);
...
void send_TO_ALL(msgbuf *message) {
    ...
    for (int i = 0; i < MAX_NO_USERS; i ++) {
        if (clients_PID[i] != 0 && i != message->sender_pid) {
...
void send_TO_ONE(msgbuf *message) {
    ...
    message_to_send.sender_pid = message->sender_pid;
...
void menage_LIST(msgbuf *message) {
    msgbuf message_to_send;
    message_to_send.sender_pid = getpid();
    ...
    if (msgsnd(clients_queue_ID[message->sender_pid], &message_to_send, size, 0) == -1) {
        perror("Can't send LIST\n");
        exit(1);
    }

    kill(clients_PID[message->sender_pid], SIGUSR1);
}
